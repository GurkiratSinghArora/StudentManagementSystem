...
{
	assert(index>=0 && index<this->A1.getSize());
	return A1[index];
}
template <class K, class V>
V Map<K,V>::getValueAtIndex(const int& index)const
{
	assert(index>=0 && index<this->A2.getSize());
	return A2[index];
}
template <class K, class V>
...
{
	if(index >= 0 && index < this->A1.getSize())
	{
		A1.remove(index);
		A2.remove(index);
		return true;
...
Course::Course(const string& str, const int& num)
{
	name=str;
	creditHours=num;
}
...
void Course::setCourseName(const string& str)
{
	name=str;
}
void Course::setCreditHours(const int& num)
{
	creditHours=num;
}
...
void Student::setFirstName(const string& str)
{
	fn=str;
}
void Student::setLastName(const string& str)
{
	ln=str;
}
...
int SchoolManagementSystem::findStudent(const string &firstName, const string &lastName) const
{
	...
	{
		if((this->studentList.operator[](i)).getFirstName()==firstName && this->studentList.operator[](i).getLastName()==lastName)
			return i;
	}
	return -1;
}
...
int SchoolManagementSystem::findCourse(const string &courseName) const
{
	...
	{
		if((this->courseList.operator[](i)).getCourseName()==courseName)
			return i;
	}
	return -1;
}
...
double SchoolManagementSystem::getStudentGPA(const int &studentIndex) const
{
	...
	assert(studentIndex>=0 && studentIndex<this->studentList.getSize());
	if((this->studentMapList.operator[](studentIndex)).getSize()==0)
		return 0.0;
	for(int i=0;i<(this->studentMapList.operator[](studentIndex)).getSize();i++)
	{
		if((this->studentMapList.operator[](studentIndex)).getKeyAtIndex(i)=='N')
		{
			countNGrade++;
		}
	}
	if(countNGrade==(this->studentMapList.operator[](studentIndex)).getSize())
		return 0.0;
	for(int i=0;i<(this->studentMapList.operator[](studentIndex)).getSize();i++)
	{
		char letterGrade=(this->studentMapList.operator[](studentIndex)).getValueAtIndex(i);
		int indexOfCourse=(this->studentMapList.operator[](studentIndex)).getKeyAtIndex(i);
		int creditHours= (this->courseList.operator[](indexOfCourse)).getCreditHours();
		if(letterGrade=='A')
		{
			creditPoints=4.0;
			totalCreditHours=totalCreditHours + creditHours;
			totalCreditPoints=totalCreditPoints + creditHours*creditPoints;
		}
...
{
	assert(this->studentList.getSize()>0);
	int maxGPAIndex=0;
	double maxGPA=this->getStudentGPA(0);
	for(int i=0;i<this->studentList.getSize();i++)
	{
		if(this->getStudentGPA(i)>maxGPA)
		{
			maxGPA=this->getStudentGPA(i);
			maxGPAIndex=i;
		}
	}
	return maxGPAIndex;
...
bool SchoolManagementSystem::registerStudent(const Student &s)
{
	for(int i=0;i<this->studentList.getSize();i++)
	{
		if(this->studentList.operator[](i)==s)
			return false;
	}
	this->studentList.append(s);
	StudentMap x;
	this->studentMapList.append(x);
	return true;
}
bool SchoolManagementSystem::enrolStudent(const int &studentIndex, const int &courseIndex)
{
	...
	for(int i=0;i<this->studentMapList.operator[](studentIndex).getSize();i++)
	{
		if(this->studentMapList.operator[](studentIndex).getKeyAtIndex(i)== courseIndex)
			return false;
	}
	this->studentMapList.operator[](studentIndex).append(courseIndex,'N');
	return true;
}
bool SchoolManagementSystem::assignLetterGrade(const int &studentIndex, const int &courseIndex, const char &letterGrade)
{
	...
	for(int i=0;i<this->studentMapList.operator[](studentIndex).getSize();i++)
	{
		if(this->studentMapList.operator[](studentIndex).getKeyAtIndex(i)== courseIndex)
		{
			this->studentMapList.operator[](studentIndex).setValueAtIndex(i,letterGrade);
			return true;
		}
	}
	return false;
...
{
	if(this->courseList.find(course)!=-1)
		return false;
	else
	{
...
bool SchoolManagementSystem::withdrawStudent(const int &studentIndex, const int &courseIndex)
{
	...
	for(int i=0;i<this->studentMapList.operator[](studentIndex).getSize();i++)
	{
		if((this->studentMapList.operator[](studentIndex)).getKeyAtIndex(i)== courseIndex)
		{
			this->studentMapList.operator[](studentIndex).remove(i);
			return true;
		}
	}
	return false;
...
void SchoolManagementSystem::removeCourse(const int &courseIndex)
{
	...
	{
		for(int j=0;j<this->studentMapList.operator[](i).getSize();j++)
		{
			if(this->studentMapList.operator[](i).getKeyAtIndex(j)>courseIndex)
				this->studentMapList.operator[](i).setKeyAtIndex(j,this->studentMapList.operator[](i).getKeyAtIndex(j)-1);
		}
	}
}
